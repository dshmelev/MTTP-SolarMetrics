...
if ($offset == 'last') {
  ...
  $num = '1 DAY';   $group = 'day(timestamp), hour(timestamp)';
} elseif ($offset == '3day') {
...
$avg_query = "SELECT timestamp, ROUND(AVG(value)) as value FROM SolarMetrics GROUP BY year(timestamp),month(timestamp),day(timestamp),hour(timestamp)";
